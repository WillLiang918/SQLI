...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    FROM
      routes AS ra
    JOIN
      routes AS rb ON (ra.num = rb.num AND ra.company = rb.company)
    JOIN
      stops AS sa ON (sa.id = ra.stop_id)
    JOIN
      stops AS sb ON (sb.id = rb.stop_id)
    WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    ra.num, ra.company, sb.name, rc.num, rc.company
  FROM
    routes AS ra
  JOIN
    routes AS rb ON (ra.num = rb.num AND ra.company = rb.company)
  JOIN
    routes AS rc ON (rb.stop_id = rc.stop_id)
  JOIN
    routes AS rd ON (rc.num = rd.num AND rc.company = rd.company)
  JOIN
    stops AS sa ON (sa.id = ra.stop_id)
  JOIN
    stops AS sb ON (sb.id = rb.stop_id)
  JOIN
    stops AS sc ON (sc.id = rd.stop_id)
  WHERE
